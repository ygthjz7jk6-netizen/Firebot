...
"""
        ...
        "social_media": {
            "model": os.getenv("MODEL_MARKETING", "gemma2:9b"),
            "temperature": 0.2,
            "prompt": """Jsi expert na virální trendy na sociálních sítích (TikTok a Instagram).
DŮLEŽITÉ: Odpověz POUZE platným JSON polem příslušné akce. Žádný text kolem.

Dostupné akce:
1. {"action": "check_tiktok", "topic": "hledaný hashtag bez #"}
2. {"action": "check_ig", "topic": "hledaný hashtag bez #"}
"""
        },
        "woocommerce": {
...
def route_to_specialist(task: str) -> str:
    ...
    if any(w in task_lower for w in ["tiktok", "instagram", "ig", "virál", "trendy", "sociální", "social"]):
        return "social_media"
